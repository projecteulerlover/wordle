...
struct Constraints {
  ...
  // The set of letters the answer MUST contain (used to deal with yellow
  // results).
...
class Wordle {
 public:
  ...
  // 2. Prompts the user for the guess the made, and the result from the UI. The
  // result
  ...
  // 3. Uses the given `result` to update `constraints_`. Uses the updated
  // `constraints`
  //    to update whether words in `candidates_` can still be valid solutions
  //    (but does not prevent them from being guessed).
 ...
 private:
  ...
  // Uses `constraints` to modify whether a word in `candidates_` is still in
  // the running as the answer.
  void UpdateCandidates();

  // Scores all the words in `candidates_`, regardless of whether or not they
  // can still be the answer, and returns the word with the highest score.
  std::pair<std::string, double> GetBestGuess();

  // Checks if the user inputs for `guess` and `result` are syntactically
  // correct, otherwise prompts the user again for these values.
