...
{
    for (auto iter = candidates_.begin(), last = candidates_.end(); iter != last; ++iter)
    {
...
{
    UpdateCandidates();
    int candidate_size = count_if(candidates_.begin(), candidates_.end(), [](const pair<string, bool> &candidate)
